...
def sac1(alpha):
    ...
    for idA in range(numOfVertices):
        for idB in range(idA, numOfVertices):
            attrA = []
            attrB = []
    ...
    if numOfEdges > 0:
        ...
        for phaseOneIteration in range(0, 15):
            print('\t Phase One iteration: {}'.format(phaseOneIteration + 1))
            convergence = True
...
def main():
    ...
    for iteration in range(0, 15):
        print("Main iteration: {}".format(iteration + 1))
        if iteration != 0:
